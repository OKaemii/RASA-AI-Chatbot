...
import textract
from .BestFile2UseAlg import BestFile2UseAlg

from os import listdir
from os.path import isfile, join

class RefactoredSearch:
    bestfile = BestFile2UseAlg()
    mypath = r"files"

    def bestFile(self, mypath, *argv):
        counter = 0
        max_val = 0
        best = ""
    ...
    #An algorithm that finds the most relevant keyword based on number of occurrences
    #then sorts which occurrences are most relevant based on proximity of other key words
    def relevanceRatio(self, text, *argv):
        rel = {} # relevance
    ...
    def searchFile(self,*argv):
        arglist = []
        for word in argv:
            word = str(word.lower())
            arglist.append(word)
        # Find the most relevent file to use
        filename = self.bestfile.cosine_similarity(10, " ".join(arglist))
        #print("filename: {}".format(filename))
        text = textract.process(filename).decode("utf-8")
        text = str(text)
        
        # Calculate the relevance ratio
        rel = self.relevanceRatio(text.lower(), *argv)
        
        # Number of results to return.
        n = 1
        
        #label each occurrence from 1 to n based on relevance using ratio algorithm
        for i in range(n):
            index = list(rel.keys())[i]
            text = text[:index] + " @" + str(i+1) + "@ " + text[index:]
        
        #print 5 lines above and 5 lines below the occurrence
        searchlines = self.readLine(text)
